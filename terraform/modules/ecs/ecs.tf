...
======*/
resource "aws_ecr_repository" "devopschallenge_app" {
...
data "template_file" "web_task" {
  ...
  vars {
    image           = "${aws_ecr_repository.devopschallenge_app.repository_url}"
    log_group       = "${aws_cloudwatch_log_group.devopschallenge.name}"
  }
}
...
resource "aws_alb_target_group" "alb_target_group" {
  ...
  depends_on = ["aws_alb.alb_devopschallenge",]
...
resource "aws_alb" "alb_devopschallenge" {
  name            = "${var.environment}-alb-devopschallenge"
...
resource "aws_alb_listener" "devopschallenge" {
  load_balancer_arn = "${aws_alb.alb_devopschallenge.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_devopschallenge_web_cpu_utilization_high"
